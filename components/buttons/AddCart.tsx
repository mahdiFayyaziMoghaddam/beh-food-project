...
export default function AddCart({classNameProp,size,onClickProp}: any) {
  ...
  return (
    ...
    <button className={`flex justify-center items-center ${size === 'lg' ? ('w-12 h-12') : ('w-6 h-6')} ${onHover ? ('bg-[#F6510B] border-[#FFFFFF]') : ('bg-white border-[#F6510B]')} ${size === 'lg' ? ('border-[0.4px] rounded-[10px]') : ('border-[0.2px] rounded-[5px]')} cursor-pointer`} onMouseOver={() => setOnHover(true)} onMouseLeave={() => setOnHover(false)} onClick={() => onClickProp()}>
