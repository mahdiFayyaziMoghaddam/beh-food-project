...
export default function Cart({onAddClick, name , title, image, price, prevPrice}: any) {

  // props: {name: string, title: string, image: string, price: number}
  ...
  return (
    ...
    <div className="flex relative mr-[1px] w-[426px] h-[132px] rounded-[10px] bg-white border-[0.2px] border-[#00000040] font-Vazir">
      ...
      <p className="w-full h-[25px] font-semibold text-[16px] leading-[25px] text-end">{name}</p>
      ...
      <AddCart classNameProp="absolute bottom-3 left-[15px]" size='sm' onClickProp={onAddClick}/>
    </div>

    {
      ...
      <Image className="absolute top-[20px] right-[19px] rounded-[50px] border-[#00000040] border-[0.2px]" width={500} height={500} src={'pizza.svg'} alt="Food"/>

        <div className="flex flex-col justify-between absolute top-[529px] left-[19px] w-[500px] h-[75px] text-right">
      <p className="w-full h-[25px] font-extrabold text-[20px] leading-[25px] text-end">پیتزا پپرونی</p>
      <p className="w-full h-[39px] font-normal text-[12px] leading-[18.75px] text-right overflow-hidden text-ellipsis" style={{direction: 'rtl'}}>خمیر پیتزا، پنیر پیتزا، سوسیس پپرونی، گوجه فرنگی، فلفل دلمه‌ای، قارچ، نمک، فلفل، آویشن و جوز هندی
      </p>
      </div>

      <div className="flex flex-row-reverse justify-start absolute bottom-[31px] right-[19px] w-[140px] h-[31px] gap-1 ">
        <span className="text-[20px] font-bold text-center leading-[31px]">17,500</span>
        <span className="text-[16px] font-medium select-none text-center leading-[31px]">تومان</span>
        </div>

        <AddCart classNameProp="absolute bottom-[23px] left-[19px]" size='lg' onClickProp={onAddClick}/>
        </Box>
